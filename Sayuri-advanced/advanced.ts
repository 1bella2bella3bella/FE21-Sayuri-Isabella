...
let car : Array<Vehicle> = [vehicle1, vehicle2,vehicle3,vehicle4];
for (let vehicle of car) {
    (document.getElementsByClassName("car")[0] as HTMLElement).innerHTML += vehicle.printInfo();
}
...
//click
function detailInfo() {
    (document.getElementsByClassName("detail")[0] as HTMLElement).innerHTML += vehicle1.clickInfo();
}

let btns =(document.getElementsByClassName("btn") as HTMLElement);
console.log(btns);

for(let i = 0; i<btns.length; i++){
    btns[i].addEventListener("click", function(){
        (document.getElementsByClassName("card-body") as HTMLElement)[i].innerHTML += car[i].clickInfo();  })
}
// for (let val of btns) {
...
class Motorbikes extends Vehicle {
    cc : number;
    constructor (model: string, kilometers: number, seats: number, fuel: string, year: number, img: string, cc: number){
...
class Trucks extends Vehicle {
    feature: string;
    constructor (model: string, kilometers: number, seats: number, fuel: string, year: number, img: string, feature: string){
        super (model, kilometers, seats, fuel, year, img)
