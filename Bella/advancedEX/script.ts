...
class Vehicles implements Machine {
    model: string;
    brand:string;
    color: string;
    kilometers: number;
    ...
    constructor (model:string, brand: string, color: string, km:number, fuel:string, year:number, image:string){
        this.model=model;
        this.brand=brand;
        this.color=color;
        this.kilometers=km;
...
class Motorbikes extends Vehicles {
    ...
    constructor(model:string, brand: string, color: string, km:number, fuel:string, year:number, image:string, numSeats:number, numWheels:number){
...
class Trucks extends Vehicles {
    loadCapacity: number;
    constructor (model:string, brand: string, color: string, km:number, fuel:string, year:number, image:string, loadCap:number){
        super(model, brand, color, km, fuel, year, image)
...
(document.getElementById("box") as HTMLElement).innerHTML += vehicle1.wholeCard();
(document.getElementById("box") as HTMLElement).innerHTML += vehicle2.wholeCardMotor();
(document.getElementById("box") as HTMLElement).innerHTML += vehicle3.wholeCardTrucky();
(document.getElementById("box") as HTMLElement).innerHTML += vehicle4.wholeCard();
